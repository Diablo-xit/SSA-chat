// script.js - Facebook Chat Simplifié (Version finale pour Render)
// Tout inclus : serveur, WebSocket, HTML, CSS, JS
...
app.post('/register', (req, res) => {
  ...
  users[email] = { name, email, password, avatar, isOnline: false };
...
app.post('/forgot-password', (req, res) => {
  ...
  const users = readJSON(USERS_FILE);
  if (users[email]) {
    console.log(`[Récupération] Lien envoyé à ${email} (simulé)`);
  }
  res.json({ message: 'Si votre email existe, un lien de récupération a été envoyé.' });
});
...
<body>
  <div class="auth-container">
    ...
    <div id="registerForm">
      ...
      <p class="form-link"><a href="#" onclick="showLoginForm()">Déjà un compte ? Se connecter</a></p>
    </div>
    <div id="loginForm" class="hidden">
      ...
      <p class="form-link"><a href="#" onclick="showForgotForm()">Mot de passe oublié ?</a></p>
...
// === HTML: Page de chat (avec message centré si personne en ligne) ===
...
<head>
  <meta charset="UTF-8">
  <title>Chat - Chat Simplifié</title>
  <style>
...
// === Démarrage du serveur (clé : 0.0.0.0 et process.env.PORT) ===
const PORT = process.env.PORT || 3000;
server.listen(PORT, '0.0.0.0', () => {
  console.log(\`Chat démarré sur http://0.0.0.0:\${PORT}\`);
});
